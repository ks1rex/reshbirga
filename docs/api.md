...
Deposits are manual only (2026-09-02) — Cashera and ParityPay gateway integrations were built and briefly live (2026-08-31/2026-09-01) but their code was removed by owner decision; `POST /deposits` above (the admin-reviewed `deposit_requests` flow) is the only deposit path again. `cashera_transactions`/`paritypay_transactions` tables and their RPCs remain in the DB, unused — see `docs/schema.md`.
