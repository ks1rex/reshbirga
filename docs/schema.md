...
| `cashera_transactions` | `20260831120000` | **Dead as of 2026-09-02** — Cashera gateway integration code was removed (owner decision), table/RPC left in place unused. Was: Cashera gateway deposit checkouts, `amount` in minor units (kopecks) |
| `paritypay_transactions` | `20260901120000` | **Dead as of 2026-09-02** — ParityPay (SBP) gateway integration code was removed (owner decision), table/RPC left in place unused. Was: `pay_amount` (RUB, what the customer pays) vs `credit_amount` (RUB, what actually lands in `deposited_balance`) |
...
- **`process_cashera_webhook`** / **`process_paritypay_webhook`** — dead code as of 2026-09-02, no Express caller left (`routes/cashera.js`/`routes/paritypay.js`/`utils/cashera.js`/`utils/paritypay.js` deleted). RPCs and their tables (`cashera_transactions`, `paritypay_transactions`) remain in the DB unused rather than being torn out — see the migration files (`20260831120000_cashera_transactions.sql`, `20260901120000_paritypay_transactions.sql`) if these gateways come back.
