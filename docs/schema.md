...
| `paritypay_transactions` | `20260901120000` | ParityPay (SBP) gateway deposit invoices — `pay_amount` (RUB, what the customer pays) vs `credit_amount` (RUB, what actually lands in `deposited_balance` — fixed at row-creation time, after the customer-facing commission); `parity_credited` holds ParityPay's own post-fee figure, for reconciliation only. RLS enabled, zero policies, same `service_role`-only pattern as `cashera_transactions` |
...
- **`process_paritypay_webhook`** (`20260901120000_paritypay_transactions.sql`) — same idempotent status-transition pattern as `process_cashera_webhook` above. The key difference: on transition into `'PAID'` it credits `paritypay_transactions.credit_amount` — a figure computed and stored by Express at invoice-creation time (`amount × (1 − paritypay_commission_pct/100)`) — **not** anything derived from the webhook payload's own `credited` field. That field (ParityPay's actual post-their-fee payout to the merchant) is stored separately as `parity_credited` purely so `platform_profit` on the resulting `transactions` ledger row can be computed (`parity_credited − credit_amount`, deliberately negative — the platform absorbs most of ParityPay's fee, see `docs/api.md`).
